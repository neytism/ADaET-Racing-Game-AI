...
public class AStarLite : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                //Collider2D hitCollider2D = Physics2D.OverlapBox(worldPosition, new Vector2(cellSize,cellSize), 0);
                ...
                if (hitCollider2D != null)
                {
                    ...
                    if (hitCollider2D.transform.CompareTag("AI"))
                        continue;

                    //Ignore player cars, they are not obstacles
                    if (hitCollider2D.transform.CompareTag("Player"))
                        continue;
                    
                    if (hitCollider2D.transform.CompareTag("Grass"))
                        continue;
    ...
    Vector2Int ConvertWorldToGridPoint(Vector2 position)
    {
        ...
        //Vector2Int gridPoint = new Vector2Int(Mathf.RoundToInt(transform.position.x), Mathf.RoundToInt(transform.position.y));;
        return gridPoint;
    }
    ...
    void OnDrawGizmos()
    {
        ...
        foreach (AStarNode toCheckNode in nodesToCheck)
        {
            ...
            Gizmos.DrawSphere(ConvertGridPositionToWorldPosition(toCheckNode), 0.25f);
        }
        ...
        Gizmos.DrawSphere(startPositionDebug, 0.25f);

        //Draw end position
        Gizmos.color = Color.red;
        Gizmos.DrawSphere(destinationPositionDebug, 0.25f);
    }
